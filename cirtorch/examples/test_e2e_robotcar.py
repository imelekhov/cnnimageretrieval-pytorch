...
# test options
parser.add_argument('--data-path', default='/data/datasets/RobotCar-Seasons')
...
TEST_SCENES = ["dawn", "dusk", "night", "night-rain", "overcast-summer", "overcast-winter", "rain", "snow", "sun"]
TRAIN_SCENES = ["overcast-reference"]
CAMERAS = ["left", "rear", "right"]
CAMERA_TEST_POS = "rear"
...
def main():
    ...
    print('>> {}: Create a list of database images...')

    img_path = osp.join(args.data_path, "images")
    fnames_db = []
    fnames_q = []

    for scene in TRAIN_SCENES:
        for camera_t in CAMERAS:
            src_path = osp.join(img_path, scene, camera_t)
            fnames_db += [osp.join(src_path, fname) for fname in os.listdir(src_path)]

    for scene in TEST_SCENES:
        for camera_t in CAMERAS:
            src_path = osp.join(img_path, scene, camera_t)
            fnames_q += [osp.join(src_path, fname) for fname in os.listdir(src_path)]
    ...
    for q_id in range(len(images_q)):
        img_q = images_q[q_id]
        if img_q.split('/')[1] != CAMERA_TEST_POS:
            continue
        pairs_per_q = [" ".join([img_q, images_db[db_id]]) for db_id in list(ranks_q[:args.top_n, q_id])]
        pairs_db += pairs_per_q

    with open(osp.join(args.data_path, "image_pairs_robotcar_top_" + str(args.top_n) + ".txt"), "w") as f:
